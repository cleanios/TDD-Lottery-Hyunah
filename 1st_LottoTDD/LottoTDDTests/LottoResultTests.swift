...
class LottoResultTests: XCTestCase {
    ...
    func testrankOfLottoShoulReturnNumberOfSameNumberAndPrizes_for5thWinner() {
        ...
        //When
        let result = LottoResult.rankings(lotto: testLotto)
        //Then
    ...
    func testRankingShoulReturnNumberOfSameNumberAndPrizes_for2ndWinner() {
        //Given
    ...
    func testTotalRankings() {
        //Given
        let firstLotto = Set(arrayLiteral: 16, 20, 24, 4, 36, 1)
        let secondeLotto = Set(arrayLiteral: 39, 20, 24, 4, 36, 1)
        //When
        let result = LottoResult.totalRankings(lottos: [firstLotto, secondeLotto])
        //Then
        XCTAssertEqual(result[4], 2)
        XCTAssertEqual(result[5], 0)
    }
    
    func testTotalRankings_containSameLotto() {
        //Given
        let firstLotto = Set(arrayLiteral: 16, 20, 24, 4, 36, 1)
        //When
        let result = LottoResult.totalRankings(lottos: [firstLotto, firstLotto,firstLotto])
        //Then
